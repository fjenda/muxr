<script lang="ts">
	...
	const setTrackVolume = (event: Event, trackId: number) => {
		...
		if (trackStates[trackId].mute || (anySolo && !trackStates[trackId].solo)) {
			...
			player.setTrackVolume(
				trackId,
				(event.target as HTMLInputElement).valueAsNumber * masterVolume
			);
	...
	const initMultitrack = (time: number = 0) => {
		...
		player = Multitrack.create(
			...
			],
			{
				...
				cursorColor: window.matchMedia('(prefers-color-scheme: dark)').matches
					? 'var(--text-color-dark)'
	...
	const onWheel = (e: WheelEvent) => {
		...
		if (e.ctrlKey) {
			e.preventDefault();
			let zoomValue =
				parseFloat((document.querySelector('#zoom-range') as HTMLInputElement)?.value) || 10;
...
<div class="audio-player">
	<div
		class="waveform-scroll-wrapper"
		bind:this={waveformScrollContainer}
		data-simplebar
		...
		<div class="waveform-grid">
			...
			<div class="waveform-container-wrapper" data-simplebar data-simplebar-auto-hide="false">
				...
				>
					...
					<div
						class="cursor-playhead"
						style="left: {cursorX}px"
						class:visible={showCursorPlayhead}
					></div>
				</div>
			</div>
			<Button onClick={() => trackUrls.downloadZip()} style="margin: 0 1rem;">
				Download all stems&nbsp;&nbsp;<FontAwesomeIcon icon={faDownload} />
			</Button>
		</div>
	</div>
